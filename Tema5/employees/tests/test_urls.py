...
class URLAccessTests(TestCase):
    def setUp(self):
        ...
        self.profile = EmployeeProfile.objects.create(
            user=self.user, position=self.position, workplace=self.workplace, gender="Male"
        )
    ...
    def test_private_url_redirects_anonymous(self):
        response = self.client.get(reverse("employees:employee_detail", args=[self.profile.pk]))
